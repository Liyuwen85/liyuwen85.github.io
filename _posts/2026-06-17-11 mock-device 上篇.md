...
Spring 的 `SmartLifecycle` 会按 phase 自动决定启动/关闭顺序，但**phase 是个隐式数字**，不读 Spring 源码很难推出哪些 bean 先启动。

`Runtime.getRuntime().addShutdownHook` 写出来的关闭顺序**是显式的**，看一眼就知道：先停止接收新输入（`close` 命令消费者）→ 停止业务（`stop` 服务）→ 释放资源（`close` 发布器）。

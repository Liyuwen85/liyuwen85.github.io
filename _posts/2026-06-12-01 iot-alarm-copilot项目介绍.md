...
另外，在技术实现上，也是尽量切近生产环境，所以做了多节点、高可用、数据冷热分层等功能，因此看起复杂了一点。
...
>"**变化节奏一致的代码放在一起，节奏不同的必须分开；以及受众边界**"的意思是：

>同一上下文内部逻辑，变更触发原因、迭代频率、发版周期尽量统一。
比如电商中：高频迭代有，营销活动、首页展示、用户端交互；
低频稳定的有，财务结算、对账、税费规则、底层库存核心算法；
完全不动，基础字典、通用认证、日志。
节奏差异巨大的代码放一块，会产生耦合发布、大量无用回归测试。

>一个限界上下文只服务同一类同质受众，
受众不同、业务目标不同、关注的模型字段 / 规则不同，优先拆开。
比如电商中，受众分类可分为：C 端用户、商家、仓储、财务、运营、骑手、风控、第三方渠道等。

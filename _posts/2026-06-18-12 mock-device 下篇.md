...
> 网关
>
> LwM2M → MQTT 转换层：Lwm2mDeviceSnapshot 是关键
>
> 网关下行命令：从 MQTT 转换成 LwM2M Write
>
...
设备端的协议非常多MQTT、CoAP、LwM2M、Modbus、ZigBee、私有TCP、行业总线PLC等，平台不可能为每种协议都建一条独立链路。网关的本质就是把“协议差异”在更靠近设备侧抹平，把多种协议格式的设备接入网关，网关用平台统一的协议（一般是 MQTT 或 HTTP）上报。

mock-device 工程同时演示了两种角色（iot-alarm-copilot项目的重点在后端，模拟设备尽量简化）：

- **设备角色**，上篇讲的 `MockDeviceTelemetryService`，模拟一个直接 MQTT 交互的设备；
- **网关角色**，`GatewayServerService` 等组件，作为 LwM2M server 接受 LwM2M 设备注册，把它们的数据转换成 MQTT 上报。
...
## LwM2M → MQTT 转换层：Lwm2mDeviceSnapshot

网关上行转换的核心是把 LwM2M 这种**资源寻址**型协议，转换成 MQTT 这种**topic + payload**型协议，两者抽象层不同：
...
直接把 LwM2M 上报的每条 resource 转换成一条 MQTT 消息会有问题，温度和湿度是同一台设备的不同 resource，分开上报到 MQTT 后下游要自己拼起来，这个任务不应该交给下游。

mock-device 网关的做法是引入一个**领域聚合 `Lwm2mDeviceSnapshot`**，以"设备"为粒度持有最新指标快照，凑齐了数据再发：
...
**3. `toGatewayMessage()` 是协议转换入口**

这个方法把 LwM2M 领域对象转换成 MQTT 上报的领域对象，`GatewayUplinkMessage`，加了一个 `gatewayId` 字段标记"这条数据来自哪个网关"。下游 access 拿到这个消息后能溯源到具体网关，合规和故障排查可使用此字段。

### 转发器把这三件事编排起来

`GatewayTelemetryForwarder` 负责整个转换流水线：
...
`LeshanLwm2mServer.setReportInterval` 把领域命令转换成 LwM2M 协议层的写操作：
...
GatewayServerService 校验 + 调 lwm2MServerRuntime
    ↓
LeshanLwm2mServer 转换成 LwM2M WriteRequest
    ↓
...
这条流向核心是协议适配模块之间不直接通信，全部通过共享状态间接交互。如果未来要换协议（比如 modbus 换成 OPC UA、lwm2m 换成 MQTT），**只需要改对应目录里的代码**，telemetry_state 不用动、应用主循环不用动。
...
应用层（gateway_app）调这三个函数即可，完全不需要掌握 anjay 内部的复杂调用。
